...
import android.view.*
import app.folder.example.dell.Speciality_March_Folder_2021.Main_Fragment_Animated.ProductA_Fragment_Main
...
class ProductA_Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        FT?.replace(R.id.containerView_a, ProductA_Fragment_Main())?.commit()
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            R.id.product_A_page9 -> {
                fn_checkLocation()
                val fragmentTransaction7 = FM!!.beginTransaction()
                fragmentTransaction7.replace(
                    R.id.containerView_a,
                    ProductA_Fragment_page_Nine()
                ).commit()
                true
            }
            R.id.product_A_page10 -> {
                fn_checkLocation()
                val fragmentTransaction8 = FM!!.beginTransaction()
                fragmentTransaction8.replace(
                    R.id.containerView_a,
                    ProductA_Fragment_page_Ten()
                ).commit()
                true
            }
            ...
            R.id.product_A_Complete -> {
                ...
                fragmentTransaction11.replace(R.id.containerView_a, ProductA_Fragment_Main())
                    .commit()
                true
            }
